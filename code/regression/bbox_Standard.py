...
def train_epoch(models, criterion, optimizers, dataloaders, epoch, loss_type):
    models['backbone'].train()

    for data in dataloaders['train_in']:
        ...
        if loss_type == 'L1':
            ...
            loss = coord_loss_r + 0.4 * iou_loss
        if loss_type == 'D-IoU':
            iou_loss = IoULoss(out_r, bboxs)
            diou_loss = DIoULoss(out_r, bboxs)
            loss = coord_loss_r + 0.4 * iou_loss + 0.4*diou_loss

        loss.backward()
        optimizers['backbone'].step()

def train(models, criterion, optimizers, schedulers, dataloaders, num_epochs, loss_type):
    print('>> Train a Model.')
    logs = []
    best_iou = 0
    for epoch in range(num_epochs):
        schedulers['backbone'].step()
        train_epoch(models, criterion, optimizers, dataloaders, epoch, loss_type)
        ...
        #np.savetxt('../logs.txt', logs, delimiter=',', fmt="%.4f")
...
if __name__ == '__main__':
    ...
    # Load Data
    train_in, test_in, _ = load_datasets(args.in_data_name, args.ood_data_name, args.n_shots)
    ...
    optim_backbone = optim.SGD(models['backbone'].parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.wdecay)
    ...
    # Training and test
    train(models, criterion, optimizers, schedulers, dataloaders, args.epochs, args.loss_type)
