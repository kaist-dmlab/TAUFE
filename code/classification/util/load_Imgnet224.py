...
class Imgnet224(torch.utils.data.Dataset):

    def __init__(self, ood=False, train=True, transform=None, in_class = np.arange(10)*10):
        def read_data(data_folder, train, in_class):
            if ood == False: # in-distribution !!!
                ...
                for i, c_name in enumerate(class_list):
                    ...
                    if i in ood_class:
                        files = os.listdir(data_folder + train_folder + c_name)
                        for f in files[:50]: # sampling ood data for efficient training
                            img = Image.open(data_folder + train_folder + c_name + '/' + f)
                            d = np.asarray(img)
                            if d.shape[-1] == 3:
                                data.append(d)
        ...
        #data_folder = '/home/data/ILSVRC2015/train/Data/CLS-LOC/'
