...
from util.load_CUB200_bbox import CUB200_Dataset
...
data_root = '/data/anonymized/CUB/'
metadata_root = '../wsolevaluation/metadata/CUB/'

# in-distribution
CUB_train_in = CUB200_Dataset(data_root=data_root,metadata_root=os.path.join(metadata_root, 'train'),transform=dataset_transforms['train'],n_shot=SAMPLE_PER_CLASS)
CUB_test = CUB200_Dataset(data_root=data_root,metadata_root=os.path.join(metadata_root, 'test'),transform=dataset_transforms['test'],n_shot=None)
...
def train_epoch(models, criterion, optimizers, dataloaders, epoch, weight):
    ...
    for (data, ood) in zip(tqdm(dataloaders['train_in'], leave=False, total=len(dataloaders['train_in'])),
        ...
        loss = coord_loss_r + 0.4*iou_loss + weight*Blackhole_loss #+ 0*diou_loss
        #print(coord_loss_r, iou_loss, Blackhole_loss) #diou_loss,
...
def train(models, criterion, optimizers, schedulers, dataloaders, num_epochs, weight):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 5 == 4:
            ...
            print('epoch: {}, acc,: {}, iou: {}'.format(epoch, acc, iou))

            # save logs
            logs.append([epoch, iou])
            #np.savetxt('../logs_sup/CUB200_localization_Blackhole_full_per_epoch_v2_place365_with_iou_loss.txt', logs, delimiter=',', fmt="%.4f")
...
train_in, train_ood = CUB_train_in, imgnet224_train_ood  # cifar100_train, imgnet224_train_ood
test_in, test_ood = CUB_test, place365_train_ood
