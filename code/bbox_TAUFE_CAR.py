...
def train_epoch(models, criterion, optimizers, dataloaders, epoch, weight):
    ...
    for (data, ood) in zip(tqdm(dataloaders['train_in'], leave=False, total=len(dataloaders['train_in'])),
        ...
        loss = coord_loss_r + weight*Blackhole_loss + 0.4*iou_loss #+ 0*diou_loss
...
def train(models, criterion, optimizers, schedulers, dataloaders, num_epochs, weight):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 5 == 4:
            ...
            #np.savetxt('../logs_sup/Car_localization_Blackhole_n2000_ImgNet_per_epoch_v1.txt',logs, delimiter=',', fmt="%.4f")
    print('>> Finished.')
...
train_in, train_ood = Car_train_in, imgnet224_train_ood  # cifar100_train, place365_train_ood, imgnet224_train_ood
test_in, test_ood = Car_test, place365_train_ood
